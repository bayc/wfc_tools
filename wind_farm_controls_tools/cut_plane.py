...
#TODO Not sure I need cropping since I can pass in arbitrary grid
        # # Make sure cropping is valid
        # if crop_x1:
        #     if crop_x1[0] < min(x1_array):
        #         raise Exception("Invalid x_1 minimum on cropping")
        #     if crop_x1[1] > max(x1_array):
        #         raise Exception("Invalid x_1 maximum on cropping")

        # if crop_x2:
        #     if crop_x2[0] < min(x2_array):
        #         raise Exception("Invalid x_2 minimum on cropping")
        #     if crop_x2[1] > max(x2_array):
        #         raise Exception("Invalid x_2 maximum on cropping")


        #     # Grid the data, if cropping available use that
        # if crop_x1:
        #     self.x1_lin = np.linspace(crop_x1[0], crop_x1[1], self.resolution[0])
        # else:
        #     self.x1_lin = np.linspace(
        #         min(self.x1_in), max(self.x1_in), self.resolution[0])
        # if crop_x2:
        #     self.x2_lin = np.linspace(crop_x2[0], crop_x2[1], self.resolution[1])
        # else:
        #     self.x2_lin = np.linspace(
        #         min(self.x2_in), max(self.x2_in), self.resolution[1])
